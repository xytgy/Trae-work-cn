...
class Enemy {
    ...
    tryDodge(incomingDirection) {
        if (!this.aiConfig || !this.aiConfig.dodgeChance) return false;
        if (this.dodgeCooldown > 0 || this.isDodging) return false;
    ...
    render() {
        ...
        // 计算闪白强度
        const flashIntensity = this.hitFlashTimer > 0 
            ? (this.hitFlashTimer / this.hitFlashDuration) * FEEDBACK.HIT_FLASH.INTENSITY 
            : 0;
