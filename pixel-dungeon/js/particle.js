...
class ParticleSystem {
    ...
    createUpgradeAura(x, y) {
        ...
        for (let i = 0; i < ringConfig.ringCount; i++) {
            ...
            this.addDelayedParticle(delay, () => {
                const expandSpeed = ringConfig.expandSpeedMin + Math.random() * 
                                  (ringConfig.expandSpeedMax - ringConfig.expandSpeedMin);
        ...
        // 向上漂浮的星星粒子 - 正常生成
        const starCount = Math.floor(starConfig.countMin + Math.random() * 
                         (starConfig.countMax - starConfig.countMin));
        for (let i = 0; i < starCount; i++) {
            const angle = -Math.PI / 2 + (Math.random() - 0.5) * Math.PI * 0.8;
            const speed = starConfig.speedMin + Math.random() * 
                         (starConfig.speedMax - starConfig.speedMin);
            const size = starConfig.sizeMin + Math.random() * 
                        (starConfig.sizeMax - starConfig.sizeMin);
            const lifetime = starConfig.lifetimeMin + Math.random() * 
                           (starConfig.lifetimeMax - starConfig.lifetimeMin);
    ...
    createMuzzleFlash(x, y, angle, color, size) {
        ...
        // 主闪光（圆形，带闪烁）
        const flash = this.createParticle(
            x, y,
            0, 0,
            '#ffaa00',
            size || config.MAIN.size,
            config.MAIN.lifetime
        );
        if (flash) {
        ...
        for (let i = 0; i < sparkCount; i++) {
            ...
            const lifetime = config.SPARK.lifetimeMin + Math.random() * (config.SPARK.lifetimeMax - config.SPARK.lifetimeMin);
            ...
            switch (particle.kind) {
                ...
                case PARTICLES.KIND.SQUARE:
                    ...
                    if (particle.isCrack) {
                        ...
                        ctx.fillStyle = particle.color;
                        ctx.fillRect(-particle.crackLength / 2, -particle.size / 2, particle.crackLength, particle.size);
                        ctx.restore();
                    } else {
                        ctx.fillStyle = particle.color;
                        ctx.fillRect(
