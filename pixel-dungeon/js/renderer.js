...
class LightingSystem {
    ...
    render(ctx, canvasWidth, canvasHeight) {
        ...
        for (const light of this.lights) {
            ...
            if (!isFinite(light.x) || !isFinite(light.y) || !isFinite(radius) || radius <= 0) {
                continue;
            }
...
class Renderer {
    ...
    handleResize() {
        ...
        if (containerAspectRatio > gameAspectRatio) {
            this.scaleFactor = containerHeight / this.height;
            this.offsetX = (containerWidth - this.width * this.scaleFactor) / 2;
            this.offsetY = 0;
        } else {
            this.scaleFactor = containerWidth / this.width;
            this.offsetX = 0;
            this.offsetY = (containerHeight - this.height * this.scaleFactor) / 2;
        }
    ...
    worldToScreen(worldX, worldY) {
        const rect = this.canvas.getBoundingClientRect();
        const x = rect.left + worldX * this.scaleFactor;
        const y = rect.top + worldY * this.scaleFactor;
        return { x, y };
    }
