...
class Player {
    ...
    useSkill(target) {
        ...
        if (skill && skill.canUse()) {
            ...
            if (skill instanceof BlinkSkill || skill instanceof ShadowStrikeSkill) {
                ...
                skill instanceof MeteorStrikeSkill ||
                skill instanceof FreezeSkill ||
                skill instanceof ThunderStrikeSkill ||
                skill instanceof LandmineSkill ||
                skill instanceof DemolitionSkill ||
                skill instanceof ThornTrapSkill ||
    ...
    updateAnimation(deltaTime) {
        ...
        if (this.weaponRecoilOffset > 0 || this.weaponRecoilRotation > 0) {
            ...
            this.weaponRecoilOffset = Math.max(
                0,
                this.weaponRecoilOffset - weaponRecoilDecay * WEAPON_ANIMATIONS.RECOIL.distance
            );
            this.weaponRecoilRotation = Math.max(
                0,
                this.weaponRecoilRotation - weaponRecoilDecay * WEAPON_ANIMATIONS.RECOIL.rotation
            );
        }
        ...
        if (this.isWeaponSwitching) {
            ...
            this.weaponSpinAngle =
                (this.weaponSwitchTimer / this.weaponSwitchDuration) * WEAPON_ANIMATIONS.SWITCH.rotateAngle;
    ...
    renderPlayer(ctx, mouseX, mouseY, isInvincible) {
        ...
        if (bodyShape) {
            ...
            ctx.beginPath();
            ctx.arc(
                -drawSize * 0.15 + eyeOffsetX * 1.5,
                -drawSize * 0.1 + eyeOffsetY * 1.5,
                drawSize * 0.06,
                0,
                Math.PI * 2
            );
