...
class UIManager {
    ...
    drawButton(ctx, x, y, width, height, text, isHovered = false, scale = 1, options = {}) {
        const config = {
            ...
            bgColor:
                options.bgColor ||
                (isHovered ? UI_COLORS.MENU_BUTTON_HOVER_START : UI_COLORS.MENU_BUTTON_GRADIENT_START),
            bgColorEnd:
                options.bgColorEnd ||
                (isHovered ? UI_COLORS.MENU_BUTTON_HOVER_END : UI_COLORS.MENU_BUTTON_GRADIENT_END),
            borderColor:
                options.borderColor || (isHovered ? UI_COLORS.MENU_BUTTON_HOVER_BORDER : UI_COLORS.MENU_BUTTON_BORDER),
        ...
        switch (state) {
            ...
            case GAME_STATE.PLAYING:
                ...
                if (this.gameLogic && this.gameLogic.minimap) {
                    this.gameLogic.minimap.render(
                        renderer.ctx,
                        this.gameLogic.dungeonLevel,
                        this.gameLogic.currentRoomNode
                    );
                }
                // 渲染新手引导
    ...
    renderDifficultySelectScreen() {
        ...
        const optionGap = 20;
        const totalHeight =
            this.difficultyOptions.length * optionHeight + (this.difficultyOptions.length - 1) * optionGap;
    ...
    renderGraphicsSettings(startY) {
        ...
        this.renderSettingOption(
            y,
            '粒子数量',
            settings.graphics.particleCount,
            ['low', 'medium', 'high'],
            ['低', '中', '高'],
            'particleCount'
        );
    ...
    renderSettingOption(y, label, value, options, labels, settingKey) {
        ...
        options.forEach((opt, i) => {
            ...
            this.settingButtons[settingKey].push({
                x: ox,
                y: y - 5,
                width: optionWidth,
                height: optionHeight,
    ...
    renderSettingToggle(y, label, value, settingKey) {
        ...
        if (!this.settingButtons) {this.settingButtons = {};}
        this.settingButtons[settingKey] = [
            { x: toggleX, y: y - 5, width: toggleWidth, height: toggleHeight, toggle: true }
        ];
    ...
    renderSettingSlider(y, label, value, min, max, settingKey) {
        ...
        this.settingSliders[settingKey] = {
            x: sliderX,
            y: sliderY,
            width: sliderWidth,
            height: sliderHeight,
    ...
    renderLeaderboardContent() {
        ...
        board.forEach((record, index) => {
            ...
            ctx.fillText(
                leaderboardManager.formatRelativeTime(record.timestamp),
                GAME_WIDTH - 120,
                y + rowHeight / 2 - 5
            );
        });
    ...
    renderAchievementsScreen() {
        ...
        try {
            ...
            const unlockedCount = allAchievements.filter(
                (a) => typeof saveManager !== 'undefined' && saveManager.isAchievementUnlocked(a.id)
            ).length;
    ...
    renderAchievementsList() {
        ...
        try {
            achievements.forEach((achievement, index) => {
                ...
                const isUnlocked =
                    typeof saveManager !== 'undefined' && saveManager.isAchievementUnlocked(achievement.id);
    ...
    renderSkillBar() {
        ...
        // 绘制冷却环
        this.drawCooldownRing(ctx, iconX + iconSize / 2, barY + iconSize / 2, iconSize / 2 + 3, cooldownPercent, skill);
    ...
    renderPassiveEffects() {
        ...
        if (passive.activeBuffs && passive.activeBuffs.length > 0) {
            ...
            for (const buff of passive.activeBuffs) {
                ...
                ctx.font = '12px "Microsoft YaHei", sans-serif';
                ctx.textAlign = 'right';
                ctx.fillText(`▲ ${buff.name}: ${Math.ceil(buff.timer / 1000)}s`, GAME_WIDTH - 10, y);
        ...
        if (passive.activeSkeletons && passive.activeSkeletons.length > 0) {
            ...
            ctx.font = '12px "Microsoft YaHei", sans-serif';
            ctx.textAlign = 'right';
            ctx.fillText(`骷髅: ${passive.activeSkeletons.length}`, GAME_WIDTH - 10, GAME_HEIGHT - 80);
        }
        ...
        if (passive.dragon && passive.dragon.active) {
            ...
            ctx.font = '12px "Microsoft YaHei", sans-serif';
            ctx.textAlign = 'right';
            ctx.fillText('飞龙: 活跃', GAME_WIDTH - 10, GAME_HEIGHT - 65);
        }
    }
