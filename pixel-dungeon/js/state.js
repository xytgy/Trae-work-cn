...
class GameStateManager {
    ...
    switchWeapon() {
        ...
        if (this.data.playerWeapons.length <= 1) {
            return false;
        }

        // 切换到下一个武器
        this.data.currentWeaponIndex = (this.data.currentWeaponIndex + 1) % this.data.playerWeapons.length;
    ...
    getRoomDifficultyMultiplier(roomIndex, isBossRoom = false) {
        ...
        if (roomIndex >= progression.length - 1) {
            return progression[progression.length - 2];
        }
